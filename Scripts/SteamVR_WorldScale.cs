...
public class TiltFive_WorldScale : MonoBehaviour {

    public TiltFive_NewController ctlMain;
    public TiltFive_NewController ctlAlt;
    ...
    private void Update() {
        if (ctlMain.gripDown || ctlAlt.gripDown) {
        ...
        } else if (ctlMain.gripped && !ctlAlt.gripped) {
        ...
        if (armed) {
            switch (sevenMode) {
                case SevenMode.BOTH:
                    ...
                    break;
                case SevenMode.MAIN:
                    attachTargetOne(ref ctlMain);
                    break;
                case SevenMode.ALT:
                    attachTargetOne(ref ctlAlt);
                    break;
            }
            armed = false;
        }
    ...
    private void attachTargetBoth() {
        initialHandPosition1 = ctlMain.transform.position;
        initialHandPosition2 = ctlAlt.transform.position;
    ...
    private void updateTargetBoth() {
        Vector3 currentHandPosition1 = ctlMain.transform.position; // current first hand position
        Vector3 currentHandPosition2 = ctlAlt.transform.position; // current second hand position
    ...
    private void attachTargetOne(ref TiltFive_NewController ctl) {
